...
class PluginManager:
    ...
    def __init__(self, config: Any, db: Any, router: Any):
        ...
        self._audit_tracker: dict[str, dict[str, int]] = {}  # {plugin_name: {tool_name: count}}
    ...
    async def process_tool_calls(self, content: str) -> tuple[str, list]:
        ...
        for match in re.finditer(pattern, content, re.DOTALL):
            ...
            # Check rate limit
            if not plugin.check_rate_limit(tool_name):
                results.append({"tool": tool_name, "error": "Rate limit exceeded"})
                continue
            ...
            try:
                ...
                # Audit hook with duration
                await self.audit_tool_call(plugin, tool_name, params, res, duration_ms)
    ...
    async def audit_tool_call(
        ...
        """Log tool call to audit trail with duration."""
        await plugin.on_after_tool_call(tool_name, params, result)
        logger.info(f"Tool call: {plugin.name}:{tool_name} (duration: {duration_ms:.2f}ms)")

        # Update audit tracker
        if plugin.name not in self._audit_tracker:
            self._audit_tracker[plugin.name] = {}
        if tool_name not in self._audit_tracker[plugin.name]:
            self._audit_tracker[plugin.name][tool_name] = 0
        self._audit_tracker[plugin.name][tool_name] += 1

    def get_audit_summary(self) -> dict[str, dict[str, int]]:
        """Return audit summary with call counts per plugin and tool.

        Returns:
            Dictionary mapping plugin_name to {tool_name: call_count}
        """
        return dict(self._audit_tracker)
